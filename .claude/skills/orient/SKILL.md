...
description: >-
  ...
  in a fresh context — "let's work on the proxy", "add a feature to the UI", "fix the data
  layer", "I want to change X" — anything that means reading and understanding code before
...
4. Then whatever the task touches:
   ...
   - **hardware** → `boards/<arch>/<slug>/SPEC.md` · **proxy** → `proxy/README.md` ·
...
(`hardware.md` for `firmware/`+`boards/`, `ui.md` for `ui/`, `proxy.md` for `proxy/`,
`release.md` for shipping). These auto-load when editing those folders — read them up front so
you know the constraints (e.g. "proxy: zero npm deps", "UI: no Arduino deps") *before* designing.
...
- **Who consumes it?** Find the other side of every contract (e.g. the firmware that parses the
  proxy's JSON, the UI that calls a `data_*()` accessor). Grep across folders for the symbols.
- **What's the contract that must stay stable** while internals change? Name it explicitly.
- **Where does config / secrets come from?** (`config.json` → build defines / proxy section /
