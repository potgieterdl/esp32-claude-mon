...
if not cfg_path:
    ...
    with open(cfg_path, encoding="utf-8") as f:
        cfg = json.load(f)
    wifi, proxy, dev = cfg.get("wifi", {}), cfg.get("proxy", {}), cfg.get("device", {})
    ...
    if "url" in proxy:  defs.append(("CFG_PROXY_URL",   S(proxy["url"])))
    if "token" in proxy: defs.append(("CFG_PROXY_TOKEN", S(proxy["token"])))
    ...
    print(f"[load_config] {os.path.relpath(cfg_path, proj)} -> injected {len(defs)} settings "
          f"(wifi='{wifi.get('ssid','?')}', proxy set, secrets redacted)")
