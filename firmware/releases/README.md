...
|------|-----|----------|--------|
| `v1.9.4.bin` | `fw-v1.9.4-good` | **I2C error-flood fix (#18).** Touch reads are gated on the CST816 `TP_INT` line (GPIO11) instead of blind ~30 Hz polling, which was flooding the log with ~1/s `[259] ESP_ERR_INVALID_STATE` from an idle controller. The INT wakes a polling burst that runs until the finger lifts, so swipe/tap still work. Root-caused by ablation (WiFi / audio / concurrency all ruled out). Measured **0/s** idle *and* while swiping (was ~0.92/s). Bumped to 1.9.4 (1.9.3 is the unmerged #2 branch). | ✅ confirmed on device (serial: 0 errors idle + swiping; touch verified by hand) |
| `v1.8.2.bin` | `fw-v1.8.2-good` | **Dev-time serial diagnostics.** New `app_diag` module: boot banner (reset reason + I2C bus scan) + ~10 s serial health line (uptime, free/min heap, RSSI, WiFi connect/disconnect counts, data age/last-error); WiFi event counters in `app_net`. USB-serial only, gated on `if(Serial)` so it's silent + non-blocking when headless. | ✅ confirmed on device (serial-validated) |
| `v1.8.1.bin` | `fw-v1.8.1-good` | **Reset-countdown fix.** "RESETS IN" ceils to whole minutes (hits `0:00` only at the real reset, not the final minute); when the 5 h window has elapsed while idle it shows a faded `--:--` + "No current session" instead of a frozen `0:00`; boundary re-poll so a fresh window's countdown returns promptly. | ✅ confirmed on device |
