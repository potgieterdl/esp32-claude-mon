...
//
// Flow: a Claude Code `Notification` hook POSTs {"event":"needs_input"} when a session is waiting on
// the user; a `UserPromptSubmit` hook POSTs {"event":"clear"} when the user resumes. The presenter
// shows a passive modal while active and chimes once on the rising edge, and enforces a safety
// timeout (via notify_input_age_s) so a missed "clear" POST can't pin the banner forever — e.g. you
// approved a permission dialog, which is not a prompt submit.
//
...
void     notify_input_set();      // raise: a Claude Code session is waiting on the user
